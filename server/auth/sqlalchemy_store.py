...
class SqlAlchemyStore:
    ...
    def get_user_by_id(self, user_id: int) -> User:
        with self.ManagedSessionMaker() as session:
            return self._get_user_by_id(session, user_id).to_mlflow_entity()
    
    @staticmethod
    def _get_user_by_id(session, user_id: int) -> SqlUser:
        try:
            return session.query(SqlUser).filter(SqlUser.id == user_id).one()
        except NoResultFound:
            raise MlflowException(
            ...
            raise MlflowException(
                f"Found multiple users with id={user_id}",
                INVALID_STATE,
            )
    ...
    def get_all_permissions_for_experiment(self, experiment_id: str) -> list[dict]:
        """Get all permission assignments for an experiment"""
        with self.ManagedSessionMaker() as session:
            # JOIN query on users and experiment_permissions tables
            result = (
                session.query(SqlUser.username, SqlExperimentPermission.permission)
                .join(SqlExperimentPermission, SqlUser.id == SqlExperimentPermission.user_id)
                .filter(SqlExperimentPermission.experiment_id == experiment_id)
                .all()
            )
            return [{"username": username, "permission": permission} for username, permission in result]

    def update_or_create_experiment_permission(
        self, experiment_id: str, username: str, permission: str
    ) -> ExperimentPermission:
        """Update or create experiment permission"""
        _validate_permission(permission)
        with self.ManagedSessionMaker() as session:
            user = self._get_user(session, username=username)
            ...
            if existing_perm:
                ...
                return existing_perm.to_mlflow_entity()
            else:
                # Create new permission
                new_perm = SqlExperimentPermission(
                    experiment_id=experiment_id, user_id=user.id, permission=permission
                )
                session.add(new_perm)
                session.flush()
                return new_perm.to_mlflow_entity()
