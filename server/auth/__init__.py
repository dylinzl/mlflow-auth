...
def validate_can_build_docker():
    """Validate if the user can build Docker images - only admins allowed"""
    # 如果这个函数被调用，说明sender_is_admin()返回了False
    # 理论上管理员用户不应该到达这里
    # 但为了调试，我们再次检查admin权限
    try:
        auth = authenticate_request()
        ...
        import logging
        _logger = logging.getLogger(__name__)
        _logger.info(f"validate_can_build_docker被调用: username={username}, is_admin={is_admin}")
...
BEFORE_REQUEST_VALIDATORS.update(
    {
        ...
        (BUILD_MODEL_DOCKER, "POST"): validate_can_build_docker,
    }
)
...
def _get_current_user_session():
    ...
    try:
        # First, try session-based auth (for browser users)
        if "username" in session and "user_id" in session:
            # Check if session has expired
        ...
        # If no valid session, try Basic Auth (for API clients accessing UI)
...
def admin_api_guard(func):
    """Admin API permission guard decorator - supports both session and Basic Auth"""
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        ...
        if auth and auth.username and auth.password:
            try:
                ...
                user = store.get_user(auth.username)
                if user and check_password_hash(user.password_hash, auth.password) and user.is_admin:
                    return func(*args, **kwargs)
            except Exception:
                pass
        ...
        response = make_response("You are not authenticated. Please login at /login to access this resource.", 401)
...
def build_model_docker():
    ...
    try:
        ...
        if not model_name or not model_version:
            return jsonify({
                "status": "error",
                "message": "缺少必需参数: model_name 和 model_version"
            }), 400
        
        # 3. 自动生成镜像名称（如果未提供）
        if not image_name:
            clean_name = model_name.lower().replace('_', '-').replace(' ', '-')
            image_name = f"{clean_name}-v{model_version}"
        
        # 4. 不设置默认基础镜像，让MLflow自动选择（与CLI行为一致）
        # 注释：不设置base_image，让MLflow PyFunc backend自动判断
        
        # 5. 验证模型存在
        try:
            model_registry_store = _get_model_registry_store()
            model_version_obj = model_registry_store.get_model_version(model_name, model_version)
        except Exception:
            return jsonify({
                "status": "error", 
                "message": f"模型版本不存在: {model_name}/{model_version}"
            }), 404
        ...
        def _simple_build():
            try:
                _logger.info(f"[{build_id}] 开始Docker构建: {model_name} v{model_version} -> {image_name}")
                ...
                try:
                    ...
                    error_msg = f"Docker服务不可用: {docker_error}"
                    if "CreateFile" in str(docker_error) or "系统找不到指定的文件" in str(docker_error):
                        error_msg = "Docker Desktop未运行，请启动Docker Desktop后重试"
                    _logger.error(f"[{build_id}] {error_msg}")
                    raise Exception(error_msg)
                
                # 2. 正确的方案：使用models:/ URI，但设置正确的环境
                import os
                import mlflow
                ...
                os.environ['MLFLOW_TRACKING_USERNAME'] = auth_config.admin_username
                os.environ['MLFLOW_TRACKING_PASSWORD'] = auth_config.admin_password
                
                _logger.info(f"[{build_id}] 配置MLflow连接")
                
                # 使用标准的models:/ URI，让MLflow自己解析
                model_uri = f"models:/{model_name}/{model_version}"
                build_params = {
                    "model_uri": model_uri, 
                    "name": image_name,
                    "env_manager": "local"  # 使用local，与您成功的CLI命令一致
                }
                
                # 只有用户明确指定base_image时才传递，否则让MLflow自动选择
                if base_image:
                    build_params["base_image"] = base_image
                
                base_image_info = base_image if base_image else "自动选择"
                _logger.info(f"[{build_id}] 开始构建 {model_name}:{model_version} -> {image_name} (基础镜像: {base_image_info})")
                
                # 调用MLflow原生API
                mlflow.models.build_docker(**build_params)
                
                _logger.info(f"[{build_id}] Docker构建成功: {image_name}")
                
            except Exception as e:
                _logger.error(f"[{build_id}] Docker构建失败: {e}")
                import traceback
                _logger.error(f"[{build_id}] 错误详情: {traceback.format_exc()}")
            finally:
                # 恢复环境
                try:
        ...
        thread = threading.Thread(target=_simple_build)
        thread.daemon = True
        thread.start()
        
        return jsonify({
            "status": "success",
            "message": "Docker镜像构建任务已启动",
            "build_id": build_id,
        ...
        _logger.error(f"Docker构建API异常: {e}")
        return jsonify({
            "status": "error",
